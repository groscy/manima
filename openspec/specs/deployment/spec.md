...
How MANIMA is stood up by someone other than its author: a render-only default
that needs only a container host (no GPU, vLLM, or Qdrant), an opt-in generate
profile, single-file env configuration, a one-command entrypoint, and a smoke
test that proves a scene actually rendered before the deployment is called
healthy. The sandbox boundary is preserved by every deployment shape.

...
The GitHub automation guarding MANIMA: the stdlib-only offline suites, lint, and
strict spec-validation on every push and pull request; render-image build
verification; release-gated image publication using the built-in workflow token;
and an honestly-drawn boundary around what a GPU-less runner cannot execute,
declared rather than faked green.
